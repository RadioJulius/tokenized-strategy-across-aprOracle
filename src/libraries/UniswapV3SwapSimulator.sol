...
library UniswapV3SwapSimulator {
    ...
    function simulateExactInputSingle(ISwapRouter router, ISwapRouter.ExactInputSingleParams memory params)
        external
        view
        returns (uint256 amountOut)
    {
        bool zeroForOne = params.tokenIn < params.tokenOut;
        IUniswapV3Pool pool = getPool(router, params.tokenIn, params.tokenOut, params.fee);
    ...
    function getPool(ISwapRouter router, address tokenA, address tokenB, uint24 fee)
        private
        view
        returns (IUniswapV3Pool)
    {
        return IUniswapV3Pool(
            IUniswapV3Factory(ISwapRouterWithFactory(address(router)).factory()).getPool(tokenA, tokenB, fee)
        );
    }
}
